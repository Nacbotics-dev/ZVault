...
import { optin_asset,TRANSACTION_SCAN_URL } from "../../ContractMethods/ContractCalls";


export default function OptInAsset({setShowPopUp}) {
    ...
    const handleSubmit = (e)=>{
        ...
        if (activeAddress) {
            ...
            optin_asset(activeAddress,signer,formData?.asset_id).then((response)=>{
                setLoading({loading:false,error:null,data:response});
            }).catch((e)=>{
                setLoading({loading:false,error:e,data:null});
            })
        }
            ...
            <div className="w-full min-h-screen fixed top-0 right-0 left-0 z-40 flex flex-col place-items-center place-content-center">
                ...
                <form onSubmit={handleSubmit} className="relative z-50 p-5 max-w-lg bg-[#fff] w-full rounded-2xl flex flex-col space-y-5">
                    ...
                    <div className="w-full grid grid-cols-3 gap-4 place-items-center md:place-items-baseline">
                        <label htmlFor="asset_id" className="font-semibold text-base md:text-lg">Asset ID</label>
                        <div className="bg-[#F6F6F6] col-span-2 w-full rounded-2xl flex place-items-center px-5 h-11 md:h-14">
                            ...
                            <input required name="asset_id" type="number" placeholder="Asset ID you want to save" className="w-full h-full px-2 bg-transparent appearance-none font-medium text-sm md:text-base"/>
                        </div>

                        {loading.error && <p className="font-medium text-center place-self-center md:text-base text-sm col-span-3 text-red-600">{loading.error?.message}</p>}
                        {loading.data && <p className="font-medium flex gap-3 w-full text-center place-content-center place-self-center md:text-base text-sm col-span-3 text-green-600">
                            Optin successful TxnID : <a href={`${TRANSACTION_SCAN_URL}${loading.data?.[0]?.txID}`} target="_blank" rel="noreferrer" className="font-bold max-w-[6rem] overflow-hidden text-ellipsis block w-full">{loading.data?.[0]?.txID}</a>
                        </p>}
                    </div>

                    <div className="w-full flex place-content-center place-items-center">
                       {activeAddress && <button disabled={loading.loading} className="rounded-2xl border-2 border-[#111] disabled:cursor-progress max-w-[11rem] w-full font-semibold text-sm md:text-lg h-11 md:h-14">
