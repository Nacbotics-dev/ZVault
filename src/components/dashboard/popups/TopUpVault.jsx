...
import { top_up_savings,TRANSACTION_SCAN_URL } from "../../ContractMethods/ContractCalls";

export default function TopUpVault({vaultInfo,setShowPopUp}) {
    ...
    const handleSubmit = (e)=>{
        ...
        if (activeAddress) {
            ...
            top_up_savings(activeAddress,signer,vault_id,asset_id,amount).then((response)=>{
        ...
        <div className="w-full min-h-screen fixed top-0 right-0 left-0 z-40 flex flex-col place-items-center place-content-center">
            ...
            <form onSubmit={handleSubmit} className="relative z-50 p-5 max-w-lg bg-[#fff] w-full rounded-2xl flex flex-col space-y-5">
                ...
                <div className="w-full grid grid-cols-3 gap-4 place-content-center md:place-items-baseline">
                    <label htmlFor="vault_id" className="font-semibold text-base md:text-lg">Vault name</label>
                    ...
                    <input readOnly type="text" name="vault_id" hidden value={vaultInfo?.vaultInfo?.vaultInfo[7]} placeholder="The name of the vault"/>
                        <input readOnly type="text" name="vault_name" defaultValue={vaultInfo?.vaultInfo?.vaultInfo[0]} placeholder="The name of the vault" className="w-full h-full px-2 bg-transparent appearance-none font-medium text-sm md:text-base"/>
                    </div>
                </div>
                ...
                {loading.data && <p className="font-medium flex gap-3 w-full text-center place-content-center place-self-center md:text-base text-sm col-span-3 text-green-600">
                            Vault Topped up TxnID : <a href={`${TRANSACTION_SCAN_URL}${loading.data?.[0]?.txID}`} target="_blank" rel="noreferrer" className="font-bold max-w-[6rem] overflow-hidden text-ellipsis block w-full">{loading.data?.[0]?.txID}</a>
                        </p>}
